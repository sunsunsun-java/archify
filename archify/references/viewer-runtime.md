...
The optional 1200×630 @2x Share Card PNG is for README, release, social, or launch previews. It preserves a standard 1200×630 layout while exporting 2400×1260 physical pixels for high-DPI displays. It uses the current theme and visual preset. When the complete diagram keeps both primary and semantic context text at or above their readability thresholds, the card retains the full canonical view. Dense ordinary cards first remove node context, fine detail, structural scaffolding, and the legend, then crop to real content. If that is still unreadable, the exporter evaluates every authored guided view and deterministic connected subsets, selecting the largest readable authored candidate before an automatic one. Relationship labels on retained edges remain semantic content. Route and reach cards retain the full authored topology and decorate only their exact resolved snapshot. This export-only simplification never claims validation and never changes the authored artifact. Copy Share Card reuses the same PNG when clipboard image writes are supported.
