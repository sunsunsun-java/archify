...
3. Artifact first: the next tool action must write the candidate. Write the candidate before inspecting renderer internals. Do not plan exact coordinates in prose. For new architecture, dataflow, sequence and lifecycle sources, set `meta.canvas_fit: "content"` and omit `meta.viewBox` unless the user requires fixed dimensions. Do not add this mode or remove fixed dimensions when editing an existing source without that intent. Start with one clear main path, short side branches, and sparse labels. Use roughly 12 primary nodes as an initial readability budget. Preserve every node and relationship required by the user's question; for larger diagrams, group related content where the selected schema supports it. Set `meta.quality_profile` to `"showcase"` unless the user explicitly requests a dense `standard` map. Start with automatic routes and labels. Do not add `via`, `channelX`, `channelY`, or `labelAt` before a diagnostic calls for one; repair one diagnosed geometry problem at a time. Before manual routing, read the relevant routing rules in `references/authoring-contract.md` and inspect `validate <type> <candidate.json> --layout-json` evidence (all five types). Check whether unnecessary agent-added controls disable automatic port spread; preserve user-required route intent. If several edges share a constrained channel, plan the smallest coupled change from measured geometry and validate it together.
...
Read [content canvas](references/content-canvas.md) when a four-type content
canvas reports capacity, origin or convergence diagnostics. A larger canvas
does not add row/stage/column slots or repair collisions. Large-world browsing
uses the existing camera; canonical export still includes the whole finite SVG.
