...
`manifest.json` binds deterministic inputs. Each schema-2 visual-check receipt binds its HTML artifact by SHA-256 and records all eight light/dark desktop viewport observations. `ablation.json` records the required destructive mutations and their exact failing contracts.

The checked-in evidence establishes functional correctness, fixed-stage independence, entry readability, reachability, canonical SVG export completeness, and the need for a zoom cap above 3x. It does not contain the Goal's matched base/candidate P4 performance matrix or a human Reader A/B study. The resulting decisions are therefore `technicalDecision: Retain experiment` and `promotionDecision: evidence-incomplete`; no user-time or Issue #175 improvement claim is made.
